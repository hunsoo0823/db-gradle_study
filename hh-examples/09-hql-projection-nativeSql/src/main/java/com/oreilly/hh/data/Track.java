...
@NamedQuery(
    ...
    query = "select track.id, track.title from Track as track where track.playTime <= :length order by track.title desc")
@NamedQuery(
    name = "com.oreilly.hh.trackSummary",
    query = "select count(*), min(track.playTime), max(track.playTime) from Track as track inner join track.artists artist where artist = :artist")
@NamedNativeQuery(
    name = "com.oreilly.hh.tracksEndingAt",
    //query = "select {track.*} from TRACK as {track} where SECOND({track}.PLAYTIME) = :seconds",
    query = "select * from TRACK as track where SECOND(track.PLAYTIME) = :seconds",
