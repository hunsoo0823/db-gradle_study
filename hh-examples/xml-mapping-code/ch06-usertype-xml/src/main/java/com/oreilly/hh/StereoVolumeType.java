...
/*
 * Hibernate Core Migration Guide : 3.6 -> 4.0
 *	https://developer.jboss.org/wiki/HibernateCoreMigrationGuide40
 *
 *		- SessionImplementor session
 *
 * From Hibernate 4.1
 * s/Hibernate.STRING/StandardBasicTypes.STRING/
 *
 * From Hibernate 4.1 -> 5.2
 * s/SessionImplementor/SharedSessionContractImplementor/
 *
 */
...
public class StereoVolumeType implements CompositeUserType {
    ...
    public Type[] getPropertyTypes() {
        return new Type[] { StandardBasicTypes.SHORT, StandardBasicTypes.SHORT };
    }
    ...
    throws SQLException {
        Short left = (Short)StandardBasicTypes.SHORT.nullSafeGet(rs, names[0], session);
        Short right = (Short)StandardBasicTypes.SHORT.nullSafeGet(rs, names[1], session);
    ...
    throws SQLException {
        if (value == null) {
            StandardBasicTypes.SHORT.nullSafeSet(st, null, index, session);
            StandardBasicTypes.SHORT.nullSafeSet(st, null, index + 1, session);
        } else {
            StereoVolume vol = (StereoVolume)value;
            StandardBasicTypes.SHORT.nullSafeSet(st, new Short(vol.getLeft()), index, session);
            StandardBasicTypes.SHORT.nullSafeSet(st, new Short(vol.getRight()), index + 1, session);
        }
    }
